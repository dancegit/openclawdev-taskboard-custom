...
@contextmanager
def get_db():
    """Database connection context manager with WAL mode for better concurrency."""
...
def log_activity(task_id: int, action: str, agent: str = None, details: str = None, conn=None):
    """Log an activity."""
    if conn is None:
        ...
        # Use existing connection (reduces lock contention)
        conn.execute(
            "INSERT INTO activity_log (task_id, action, agent, details, timestamp) VALUES (?, ?, ?, ?, ?)",
            (task_id, action, agent, details, datetime.now().isoformat())
        )
...
async def create_task(task: TaskCreate):
    ...
    with get_db() as conn:
        ...
        # Log activity using same connection to avoid lock contention
        log_activity(task_id, "created", task.agent, f"Created: {task.title}", conn=conn)
...
async def update_task(task_id: int, updates: TaskUpdate):
    ...
    with get_db() as conn:
        ...
        for field in ["title", "description", "status", "priority", "agent", "due_date", "source_file", "source_ref", "agent_session_key", "working_agent"]:
        ...
        if update_fields:
            ...
            # Log activity using same connection to avoid lock contention
            log_activity(task_id, "updated", updates.agent or current["agent"], "; ".join(changes), conn=conn)
...
async def delete_task(task_id: int):
    ...
    with get_db() as conn:
        ...
        conn.commit()
        # Log activity using same connection to avoid lock contention
        log_activity(task_id, "deleted", None, f"Deleted: {row['title']}", conn=conn)
...
async def start_work(task_id: int, agent: str):
    ...
    with get_db() as conn:
        ...
        if current_status in ["Backlog", "Blocked"]:
            ...
            moved = True
            # Log activity using same connection to avoid lock contention
            log_activity(task_id, "status_change", agent, f"Auto-moved from {current_status} to In Progress (agent started work)", conn=conn)
        else:
            conn.execute(
...
async def stop_work(task_id: int, agent: str = None, outcome: str = None, reason: str = None):
    ...
    with get_db() as conn:
        ...
        if new_status:
            ...
            # Log activity using same connection to avoid lock contention
            log_activity(task_id, "status_change", agent or "Agent", f"Auto-moved to {new_status} (agent stopped work)", conn=conn)
        else:
            conn.execute(
...
async def move_task(task_id: int, move_request: MoveRequest):
    ...
    with get_db() as conn:
        ...
        conn.commit()
        # Log activity using same connection to avoid lock contention
        log_activity(task_id, "moved", agent, f"Moved to {status}", conn=conn)
